...
#   1. Creates the following Predix Services: UAA, Asset,Time Series.
...
loginCf()
{
  ...
  read -s -p "Enter the Password provided by Instructor :" password
  cf login -a https://api.system.aws-usw02-pr.ice.predix.io -u $username -p $password -o Predix-Training || echo sadKitty
...
getUAAEndpoint() {
	...
	uaa_uri=$env_cf_app|grep predix-uaa-training|egrep -o '"uri": "https?://[^ ]+"'|sed s/\"uri\":\ //g|sed s/\"//g
...
updateClient() {
  ...
  uaac client update $clientname --authorities "clients.read clients.write scim.write scim.read acs.policies.read acs.policies.write acs.attributes.read
        acs.attributes.write idps.read idps.write uaa.resource $asset_zone timeseries.zones.$timeseries_zone.query timeseries.zones.$timeseries_zone.user timeseries.zones.$timeseries_zone.ingest " --scope "$asset_zone openid uaa.none $timeseries_zone"
}

createUsers() {
	echo ""
	echo "Creating users..."
	uaac user add app_admin --emails app_admin@asset.com -p app_admin || sadKitty
	uaac user add app_user --emails app_user@asset.com -p app_user || sadKitty
}
